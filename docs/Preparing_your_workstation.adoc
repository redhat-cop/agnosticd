...
Please see the <<configs-on-osp, OpenStack tutorial>>. 
...
aws ec2 import-key-pair --key-name ${KEYNAME} --region=$REGION --output=text --public-key-material fileb://~/.ssh/id_rsa.pub
----
+
