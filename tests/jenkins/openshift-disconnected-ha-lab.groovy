...
// Catalog items
def choices = [
    'OPENTLC OpenShift Labs / OpenShift HA Lab',
    'DevOps Deployment Testing / OCP HA Disconnected Testing',
    'DevOps Team Development / DEV OCP HA Disconnected Lab',
].join("\n")
                ...
                script {
                    ...
                    guid = sh(
                        ...
                        script: """
                          ...
                          -i '${item}' \
                          -G '${cf_group}' \
                          -d 'status=t,notes=Development - Catalog item creation / maintenance,expiration=5,runtime=8,ocprelease=${ocprelease},region=${region},nodes=${nodes}'
                        """
                    ).trim()
