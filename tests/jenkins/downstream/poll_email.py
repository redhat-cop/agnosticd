...
parser.add_argument(
    "--filter", type=str, dest="filter", help="The string to filter email Subject."
)
...
def connect():
    ...
    while retries < max_retries:
        try:
            ...
            print(
                "[%d / %d] retrying to login.. wait %d sec"
                % (retries, max_retries, 2**retries)
            )
            sleep(2**retries)
            retries += 1
...
def wait_email(pattern, time_window):
    ...
    while max_retries > 0:
        try:
            ...
            typ, data = M.search(
                None,
                '(UNSEEN (OR HEADER Subject "%s" HEADER Subject "%s_COMPLETED"))'
                % (guid, guid),
            )
            for num in data[0].split():
                ...
                if guid in subject:
                    if "has failed" in subject:
                        ...
                        disconnect(M)
                        print(
                            "ERROR failed mail received for guid=%s: %s"
                            % (guid, subject)
                        )
                        exit(2)
                    elif "has updated" in subject:
                        print(
                            "OK update mail received for guid=%s: %s" % (guid, subject)
                        )
...
print(wait_email(args.filter, args.timeout))
