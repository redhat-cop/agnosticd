...
def find_files(base_dir, exclude_dirs, include_dirs, file_regex):
    ...
    for root, dirs, files in os.walk(base_dir):
        ...
        matches = [
            os.path.join(root, f) for f in files if re.search(file_regex, f) is not None
        ]
...
def find_playbooks():
    ...
    for yaml_file in find_files(
        os.path.join(rootdir, "notactivatedyet"), exclude_dirs, None, r"\.ya?ml$"
    ):
            ...
            for task in yaml.safe_load(contents) or {}:
                ...
                if "include" in task or "import_playbook" in task:
                    ...
                    included_file = os.path.normpath(
                        os.path.join(os.path.dirname(yaml_file), included_file_name)
                    )
...
class AADYamlLint(Command):
    ...
    def finalize_options(self):
        ...
        assert self.format in ["standard", "parsable"], "unknown format {0}.".format(
            self.format
        )
...
class AADGenerateValidation(Command):
    ...
    def run(self):
        ...
        generate_files = find_files(
            "roles", ["inventory", "test", "playbooks", "utils"], None, "generate.py$"
        )

        if len(generate_files) < 1:
            print(
                "Did not find any code generation.  Please verify module code generation."
            )  # noqa: E501
            raise SystemExit(1)
...
class AADSyntaxCheck(Command):
    ...
    def deprecate_jinja2_in_when(self, yaml_contents, yaml_file):
        ...
        for item in search_results:
            try:
                ...
                print(
                    "{}Error: Type of 'when' not recognized"
        ...
        if len(failed_items) > 0:
            print(
                "{}Error: Usage of Jinja2 templating delimiters in when "
    ...
    def run(self):
        ...
        exclude_dirs = ("adhoc", ".tox", "archive", "files")
        for yaml_file in find_files(rootdir, exclude_dirs, None, r"\.ya?ml$"):
            with open(yaml_file, "r") as contents:
        ...
        if invalid_include:
            print(
                "{}Invalid included playbook(s) found. Please ensure"
                " component entry point playbooks are not included{}".format(
                    self.FAIL, self.ENDC
                )
            )
        ...
        for playbook in entrypoint_playbooks:
            ...
            if any(x in playbook for x in invalid_entry_point):
                print(
                    "{}Invalid entry point playbook or orphaned file. Entry"
            ...
            try:
                ...
                subprocess.check_output(
                    [
                        ...
                        playbook,
                        "-e",
                        "@{}_extras".format(tox_ansible_inv),
                        "-e",
                        "ANSIBLE_REPO_PATH={}".format(os.path.join(rootdir, "ansible")),
                    ],
                )
            except subprocess.CalledProcessError as cpe:
                print("{}Execution failed: {}{}".format(self.FAIL, cpe, self.ENDC))
                has_errors = True
...
class UnsupportedCommand(Command):
    ...
    def run(self):
        """run command"""
        print("Unsupported command for ansible_agnostic_deployer")


setup(
    name="ansible_agnostic_deployer",
    license="Apache 2.0",
    cmdclass={
