...
* A new custom credential type called `nios custom credentials` is created in Tower and can be used to pass nios admin/password variables.
* A new credential called `nios creds` will be added to Tower as type `nios custome credentials` with variables pre-defined for each lab deployment.
...
* A 10.0.0.0/8 network container is automatically added to NIOS.   
...
The `infoblox-client` python library requires python3 so a custom virtualenv is created in `/var/lib/awx/venv/nios` on the Tower and Bastion instances during lab deployment.  This will have the `infoblox-client` and `ansible` python packages already installed.  You will need to have this selected as your `ANSIBLE ENVIRONMENT` at the Organziation, Project, or Job Template level in order to use it. 
