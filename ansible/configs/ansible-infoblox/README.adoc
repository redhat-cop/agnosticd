...
* When adding Job Templates in Tower, use `/var/lib/awx/venv/nios` for the `ANSIBLE ENVIRONMENT` or the job will most likely fail.
...
A python3 virtualenv is created in `/var/lib/awx/venv/nios` on the Tower and Bastion instances during lab deployment.  This will have the `infoblox-client` and `ansible` python packages installed.  
