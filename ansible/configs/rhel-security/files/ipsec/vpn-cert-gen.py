...
def create_csr(
    pkey, CN, C=None, ST=None, L=None, O=None, OU=None, emailAddress=None, algo="sha256"
):
...
def add_ext(cert, kind, crit, string):
    ...
    cert.add_extensions(
        [crypto.X509Extension(kind.encode("utf-8"), crit, string.encode("utf-8"))]
    )
...
def create_sub_cert(
    ...
    O=VPN_ORG,
    OU="Clients",
    emailAddress="pwouters@redhat.com",
    ty=crypto.TYPE_RSA,
    keybits=2048,
    sign_alg="sha256",
    isCA=False,
...
def create_root_ca(
    ...
    O=VPN_ORG,
    OU="Clients",
    emailAddress="info@%s" % DOMAIN_NAME,
    ty=crypto.TYPE_RSA,
    keybits=2048,
    sign_alg="sha256",
):
...
def run_dist_certs():
    ...
    create_end_certs(cacert, cakey, ("ipsec", "ipsec2", "audit", "usbguard", "servera"))
