...
A "fork" (`cp -r ocp4-cluster hybrid-cloud`) of the ocp4-cluster config to leverage its IP and code but allow customization for Hybrid DEploys including:

* Deploying multiple OCP clusters across multiple different cloud providers (e.g. AWS, Azure etc)
* Interfacing to Poolboy and allow the _binding_ of multiple deploys into 1 


NOTE: MUch of the following README at first commit reflects the README from `config/ocp-4-cluster` 
...
* Override the defaults for your environment by creating your own my environment-variables.yml file, as below.
...
Ex: `ansible-playbook ansible/configs/hybrid-cloud/destroy_env.yml -e @myenvironment-variables.yml  -e@my-secrets.yml`
...
pip install -r ./ansible/configs/hybrid-cloud/files/admin_host_requirements.txt
----
...
cd agnosticd
git checkout disco-ocp4
pip install -r ./ansible/configs/hybrid-cloud/files/macos_requirements.txt
----
