= ocp-workshop-with-cloud-provider config

This config deploys OCP3 cluster with AWS Cloud Provider.

== DISCLAIMER

AWS Cloud provider creates resources outside of standard CloudFormation Template we create as part of this config. These include :
...
These resources are not launched by AgnosticD config. Hence, they need to be cleaned up outside of the cloudformation template. 
The CloudFormation template deletion will fail, since the new resources are out-of-band of the template.

To solve this issue, we manually delete those resources in this config. Then, we try to delete the CloudFormation template.

Please note that the failure on CloudFormation while deletion is expected. We just retry the same task by deleting other dependent resources manually.
