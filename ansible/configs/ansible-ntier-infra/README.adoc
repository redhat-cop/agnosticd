...
*ansible-ntier-infra* is a more modern derivitive from the earlier *Three Tier App* _config_  
...
== Deploying the `ansible-ntier-infra` config

You can deploy this config by running the following command from the `ansible`
directory. You will have to provide credentials and adjust settings to your own
environment.

`ansible-playbook main.yml -e @configs/ansible-ntier-infra/sample_vars_ec2.yml -e @<YOUR_SECRET_VARS.yml>`

=== To Delete an environment
----
`ansible-playbook destroy.yml -e @configs/ansible-ntier-infra/sample_vars_ec2.yml -e @<YOUR_SECRET_VARS.yml>`
