...
* This role copies a binary grading script onto the bastion host's `/usr/local/bin` named based on the `ocp4_workload_example_pntae_course_id` variable.
...
* The variable `ocp4_workload_example_pntae_course_id` must be set to the course id (For example, cee-cf-110)
