= ocp4_workload_sealed_secrets - Deploy and configure the Sealed Secrets Operator to an OpenShift Cluster

== Role overview

* This role installs the Sealed Secrets Operator into an OpenShift Cluster and then configures it. It consists of the following tasks files:
** Tasks: link:./tasks/pre_workload.yml[pre_workload.yml] - Sets up an
 environment for the workload deployment.
*** Debug task will print out: `pre_workload Tasks completed successfully.`

** Tasks: link:./tasks/workload.yml[workload.yml] - Used to deploy the operator.
