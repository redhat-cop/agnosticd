...
* Support for overlapping IP ranges across different clusters
...
| (empty)
| *Required.* Cluster name for IP allocation (supports overlapping IP ranges across clusters)
...
* **EXTERNAL_IP_BASTION**: `.14` (bastion host)
...
The role supports cluster-specific IP allocations, allowing the same allocation names to be used across different clusters with overlapping IP ranges. This is useful when:

* Running multiple OpenShift clusters that need the same IP allocation pattern
* Testing across development, staging, and production environments
* Isolating IP allocations by cluster while maintaining consistent naming
