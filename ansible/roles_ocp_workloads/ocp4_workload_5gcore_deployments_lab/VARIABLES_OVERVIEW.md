...
The `defaults/main.yml` for **ocp4_workload_5gcore_deployments_lab** defines configuration for the 5G Core lab: 1 Hub + 12 MNO VMs (3 masters, 9 workers in 3 failure domains). The MNO cluster is **not** deployed by the role; you deploy it with **ClusterInstance** and **PolicyGenerators** from the lab repo.
...
| `lab_release` | `"4.20"`       | Used for kcli RPM URL          |
...
- `install_lab_dependencies`, `download_rhcos_isos`, `disconnected_update`, `extra_disk_libvirt_images`
...
3. **MinIO** is disabled by default; ODF is used for S3 in the 5G Core lab.
