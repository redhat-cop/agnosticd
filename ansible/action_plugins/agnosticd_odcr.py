    ...
    for reservation in deepcopy(reservations):
        for r in l:
            if reservation_match(r, reservation):
                r["instance_count"] = int(r["instance_count"]) + int(
...
class ODCRFactory:
    ...
    def describe_reservations(self, region, availability_zone="*"):
        ...
        result = list(
            filter(
                lambda elem: tag_match(self.tags, elem["Tags"]),
    ...
    def create_reservation(self, region, availability_zone, reservation):
        ...
        try:
            ...
            try:
                ...
                six.raise_from(
                    AnsibleError("could not parse duration {}".format(self.ttl)), err
                )
            ...
            if "InsufficientInstanceCapacity" in str(err):
                display.display(
                    "InsufficientInstanceCapacity %s - %d * %s"
                    % (
                        availability_zone,
                        reservation["instance_count"],
                        reservation["instance_type"],
            ...
            six.raise_from(
                AnsibleError("Client Error while creating reservation."), err
            )
    ...
    def create_reservations(self, region, availability_zone, reservations):
        ...
        for reservation in reservations:
            ...
            return_ok, reservation_id = self.create_reservation(
                region, availability_zone, reservation
            )
    ...
    def do_reservation_group(self, region, reservation_group, az_done):
        ...
        all_instance_types = set(i["instance_type"] for i in reservation_group)
        ...
        for availability_zone in matching_azs:
            ...
            r_ok, reservation_ids = self.create_reservations(
                region, availability_zone, reservation_group
            )
    ...
    def __init__(
        self, aws_key, aws_secret, task_vars, ttl, dry_run=False, distinct=False
    ):
...
class ActionModule(ActionBase):
    ...
    def run(self, tmp=None, task_vars=None):
        ...
        if not aws_access_key_id or not aws_secret_access_key:
            ...
            result[
                "error"
            ] = "aws_access_key_id and aws_secret_access_key arg must be passed"
            return result
            ...
            if r_ok:
                ...
                if len(result["reservations"]) == 1:
                    for k in list(result["reservations"]):
                        result["single_availability_zone"] = result["reservations"][k][
                            "availability_zone"
                        ]
