...
def main():
        ...
        try:
            (vsphere_client, service_instance) = connect(host,
                                     login,
                                     password,
                                     insecure=True)
        ...
        if state == "present":
          ...
          try:
            folder_obj = get_folder(service_instance, dc, folder_name, parent_folder_obj)
          except:
            folder_obj = None
          ...
          module.exit_json(changed = changed_value, folder = folder_name, parent_folder = parent_folder_name)

        elif state == "absent":

          if not parent_folder_obj:
            module.exit_json(changed = False, folder = folder_name, parent_folder = parent_folder_name)
