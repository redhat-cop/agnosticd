    ...
    if upload_element == "vms":
        for element in elements:
            ...
            if vm.vm_state != "stopped":
                module.fail_json(
                    msg="VM %s is not in stopped state, state is %s"
                    % (vm.name, vm.vm_state)
                )
            dest_disk = "%s/%s" % (output_dir, element["dest"])
            src = "rbd:%s/%s_disk" % (glance_pool, vm.id)
            upload_objects += (
                check_convert_and_upload_object(module, src, element, dest_disk) + "\n"
            )
...
def check_convert_and_upload_object(module, src, element, dest_disk):
    ...
    try:
        ...
        module.fail_json(
            msg="Object %s/%s on IBM Cloud exists" % (project, element["dest"])
        )
        ...
        s3.meta.client.upload_file(
            dest_disk, bucket_name, "%s/%s" % (project, element["dest"])
        )
        os.remove(dest_disk)
