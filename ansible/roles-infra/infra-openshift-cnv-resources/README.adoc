...
- `instances` (list): Each item:
  ...
  - `inputs` (list, optional): passed to `spec.template.spec.domain.devices.inputs` as JSON (KubeVirt input devices).
  - `host_devices` (list, optional): passed to `spec.template.spec.domain.devices.hostDevices` as JSON.
  - `tpm` (optional): KubeVirt emulated vTPM under `spec.template.spec.domain.devices.tpm`.
    Set to boolean `true` to add `tpm: {}` (ephemeral TPM state; typical for labs).
    Set to a mapping (e.g. `persistent: true`) to pass through verbatim; persistent TPM requires cluster support (KubeVirt `VMPersistentState` and `vmStateStorageClass`—see OpenShift Virtualization / KubeVirt documentation).
  - `services` (list): see service item below
  - `routes` (list): see route item below
