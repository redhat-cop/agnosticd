= ocp4-workload-open-data-hub - Deploy basic Open Data Hub environment for workshops
...
----
for i in $(oc get nodes --selector=node-role.kubernetes.io/worker --no-headers | cut -d\  -f1); \
do \
    oc debug node/"$i"
done
----
