# To build this stack:
# docker build -t quay.io/openshiftlabs/quarkus-workshop-stack:3.20 -f stack.Dockerfile .
# docker push quay.io/openshiftlabs/quarkus-workshop-stack:3.20
...
RUN cd /tmp && git clone https://github.com/RedHat-Middleware-Workshops/quarkus-workshop-m3-labs && cd quarkus-workshop-m3-labs && git checkout rhbq-3.20 && for proj in *-petclinic* ; do mvn -fn -f ./$proj dependency:resolve-plugins dependency:resolve dependency:go-offline clean compile -DskipTests ; done && cd /tmp && rm -rf /tmp/quarkus-workshop-m3-labs
