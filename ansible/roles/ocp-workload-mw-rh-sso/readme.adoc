...
$ echo "export rhsso_url=https://`oc get route sso -n $rhsso_project --template "{{.spec.host}}" -n $rhsso_project`" >> ~/.bashrc
$ source ~/.bashrc
-----
...
. Add realm certificate to a Java truststore:
...
$ keytool -import
          ...
          -alias realm1.ca \
          -keystore /tmp/$rhsso_realm_truststore.jks \
          -storepass ratwater
-----
